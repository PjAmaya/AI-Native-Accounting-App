...
export type CreditNoteFormOptions = {
  clients: Option[];
  invoices: { value: string; label: string; contactId: string }[];
  projects: Option[];
...
export function CreditNoteForm({ options }: { options: CreditNoteFormOptions }) {
  ...
  const clientInvoices = options.invoices.filter((i) => i.contactId === contactId);
    ...
    <form action={action} className="grid gap-5">
      <section className="card px-6 py-5">
        <div className="grid grid-cols-3 gap-4">
          <div>
            <label htmlFor="contactId" className="eyebrow">Client</label>
            <select
              id="contactId"
              name="contactId"
              value={contactId}
          ...
          <div>
            <label htmlFor="originalInvoiceId" className="eyebrow">Credits invoice</label>
            <select
        ...
        <div className="mt-4">
          ...
          <input
            ...
            placeholder="Billing error on the October retainer"
            className={`${inputClass} mt-1.5`}
            required
          />
          {err("reason") ? (
