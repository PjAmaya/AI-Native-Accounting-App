...
import { voidBillAction } from "@/app/(app)/bills/actions";
...
function ConfirmButton() {
  ...
  return (
    ...
    >
      {pending ? "Voiding..." : "Void this bill"}
    </button>
  );
}

export function VoidBill({ billId, billNumber }: { billId: string; billNumber: number }) {
  const [open, setOpen] = useState(false);
  const action = voidBillAction.bind(null, billId);
  ...
  return (
    <div className="card w-96 border-tint-amber bg-surface p-4 shadow-lg">
      <div className="flex items-start justify-between gap-3">
        <p className="text-[13px] font-semibold">Void bill #{billNumber}</p>
        <button
          type="button"
      ...
      <p className="mt-1.5 text-[12px] leading-snug text-muted">
        The bill keeps its number and stays on record. Its journal entry is reversed, so it leaves
        payables and every report. This cannot be undone.
      </p>
      ...
      <form action={action} className="mt-3">
        ...
        <input
          ...
          required
          autoFocus
          placeholder="Duplicate of bill #4"
