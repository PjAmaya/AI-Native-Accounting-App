...
export function VoidInvoice({
  ...
  const action = voidInvoiceAction.bind(null, invoiceId);
  ...
  return (
    <div className="card w-96 border-tint-amber bg-surface p-4 shadow-lg">
      <div className="flex items-start justify-between gap-3">
        <p className="text-[13px] font-semibold">Void {invoiceNumber}</p>
        <button
          type="button"
      ...
      <p className="mt-1.5 text-[12px] leading-snug text-muted">
        The invoice keeps its number and stays on record. Its journal entry is reversed, so it
        leaves every report. This cannot be undone.
      </p>
      ...
      <form action={action} className="mt-3">
        ...
        <input
          ...
          required
          autoFocus
          placeholder="Issued to the wrong client"
